...
LAST_VERSION=$(curl --silent "${MIRROR}https://api.github.com/repos/snail007/proxy-admin-commercial/releases/latest" | grep -Po '"tag_name": *"\K.*?(?=")')
wget  -t 1 "${MIRROR}https://github.com/snail007/proxy-admin-commercial/releases/download/${LAST_VERSION}/$F"
